...
public class OAuthClient {
    public static String SERVER_ROOT = MyConstants.SERVER_ROOT;
    public static String AUTH_SERVER_ROOT  = SERVER_ROOT + "/auth";
//    public static String APP_ROOT;
//    public static String APP_CALLBACK_ROOT;
    ...
    public static void updateURLs() {
        SERVER_ROOT = MyConstants.SERVER_ROOT;
        AUTH_SERVER_ROOT = SERVER_ROOT + "/auth";
//        updateAppRootRealm("master");
    }

//    public static void updateAppRootRealm(String realm) {
//        APP_ROOT = ResteasyProviderFactory.getInstance().getContextData(HttpRequest.class).getUri().getBaseUri().toString();
//        APP_CALLBACK_ROOT = APP_ROOT + "login-callback";
//    }
...
//    private WebDriver driver;
    ...
    public class LogoutUrlBuilder {
        private final UriBuilder b = UriBuilder.fromUri(Services.instance().getSession().getAuthServerInfo().getLogoutEndpoint());
...
//
//        private final UriBuilder b = MyOIDCLoginProtocolService.logoutUrl(UriBuilder.fromUri(baseUrl)).path(backchannelLogoutPath);
//
//        public String build() {
//            return b.build(realm).toString();
//        }
...
//    }
    ...
    public WebResponse<List<NameValuePair>, AccessTokenResponse> doAccessTokenRequest(String code, String password) {
        try (CloseableHttpClient client = httpClient.get()) {
    ...
    // KEYCLOAK-6771 Certificate Bound Token
    public WebResponse<List<NameValuePair>, AccessTokenResponse> doAccessTokenRequest(String code, String password, CloseableHttpClient client) {
        ...
        if (redirectUri != null) {
            ...
            parameters.add(new BasicNameValuePair(OAuth2Constants.REDIRECT_URI, new UrlBean().getClientRedirectUri()));
        }
        ...
        try {
            return new WebResponse<>(parameters, new AccessTokenResponse(client.execute(post)));
        } catch (Exception e) {
...
//    }
    ...
    public WebResponse<String, OIDCConfigurationRepresentation> doWellKnownRequest(String realm) {
        log.infof("Sending request to well-known endpoint. Base URL: %s, Realm: %s", baseUrl, realm);
        try (CloseableHttpClient client = MutualTLSUtils.newCloseableHttpClientWithDefaultKeyStoreAndTrustStore()) {
            String wellKnownEndpoint = baseUrl + "/realms/" + realm + "/.well-known/openid-configuration";
            SimpleHttp request = SimpleHttp.doGet(wellKnownEndpoint, client);
            if (requestHeaders != null) {
                for (Map.Entry<String, String> entry : requestHeaders.entrySet()) {
                    request.header(entry.getKey(), entry.getValue());
                }
            }
            return new WebResponse<>(wellKnownEndpoint, request.asJson(OIDCConfigurationRepresentation.class));
        } catch (IOException ex) {
            throw new RuntimeException(ex);
        }
    }
        ...
        public ParResponse(CloseableHttpResponse response, Consumer<CloseableHttpResponse> c) throws Exception {
            try {
                ...
                if (!"application/json".equals(contentType)) {
                    throw new RuntimeException("Invalid content type. Status: " + statusCode + ", contentType: " + contentType);
                }
...
//    }
    ...
    public void openLogout() {
        UriBuilder b = UriBuilder.fromUri(Services.instance().getSession().getAuthServerInfo().getLogoutEndpoint());
    ...
    public String getLoginFormUrl() {
        UriBuilder b = UriBuilder.fromUri(Services.instance().getSession().getAuthServerInfo().getAuthorizationEndpoint());
        if (responseType != null) {
...
//    }
    ...
    public String getAccessTokenUrl() {
        return Services.instance().getSession().getAuthServerInfo().getTokenEndpoint();
    }

    public String getTokenIntrospectionUrl() {
        return Services.instance().getSession().getAuthServerInfo().getIntrospectionEndpoint();
    }
...
//    }
    ...
    public String getTokenRevocationUrl() {
        return Services.instance().getSession().getAuthServerInfo().getRevocationEndpoint();
    }

//    public String getResourceOwnerPasswordCredentialGrantUrl() {
//        UriBuilder b = MyOIDCLoginProtocolService.tokenUrl(UriBuilder.fromUri(baseUrl));
//        return b.build(realm).toString();
//    }
//
...
//    }
    ...
    public String getCertsUrl(String realm) {
        return Services.instance().getSession().getAuthServerInfo().getJwksUri();
    }
//
...
//
    ...
    public String getUserInfoUrl() {
        return Services.instance().getSession().getAuthServerInfo().getUserinfoEndpoint();
    }
...
//    }
    ...
    public String getParEndpointUrl() {
        return Services.instance().getSession().getAuthServerInfo().getPushedAuthorizationRequestEndpoint();
    }
